...
import { ArrayType, BuiltInTypeKind, DiscriminatedObjectType, ObjectTypeProperty, ObjectTypePropertyFlags, ObjectType, ResourceFlags, TypeFactory, TypeReference, TypeBaseKind } from "bicep-types";
...
export function generateTypes(host: AutorestExtensionHost, definition: ProviderDefinition) {
  ...
  function processResource(fullyQualifiedType: string, definitions: ResourceDefinition[]) {
    if (definitions.length > 1) {
      ...
      const discriminatedBodyType = factory.addDiscriminatedObjectType('name', fullyQualifiedType, {}, polymorphicBodies)
  ...
  function generateTypes() {
    ...
    for (const fullyQualifiedType in resourcesByType) {
      ...
      // TODO: somethings wrong here with the flags 
      const { descriptor, bodyType } = output;
      let flags = ResourceFlags.None;

      factory.addResourceType(`${getFullyQualifiedType(descriptor)}@${descriptor.apiVersion}`, descriptor.scopeType, undefined, bodyType, flags);
    }
  ...
  function getStandardizedResourceProperties(descriptor: ResourceDescriptor, resourceName: TypeReference): Dictionary<ObjectTypeProperty> {
    ...
    return {
      id: createObjectProperty(factory.addStringType(), ObjectTypePropertyFlags.ReadOnly | ObjectTypePropertyFlags.DeployTimeConstant | ObjectTypePropertyFlags.Identifier, 'The resource id'),
  ...
  function createObject(definitionName: string, schema: ObjectSchema, properties: Dictionary<ObjectTypeProperty>, additionalProperties?: TypeReference) {
    if (schema.discriminator) {
      return factory.addDiscriminatedObjectType(definitionName, schema.discriminator.property.serializedName, properties, {});
    }
  ...
  // function isDiscriminatedObjectType(type: BicepType): type is DiscriminatedObjectType {
  //   return (typeof (type as DiscriminatedObjectType).name === 'string' ) && 
  //   (typeof (type as DiscriminatedObjectType).discriminator === 'string' ) &&
  //   ((type as DiscriminatedObjectType).elements === Record<string, TypeReference> );
  // }
  ...
  function handlePolymorphicType(discriminatedObjectType: DiscriminatedObjectType, putSchema?: ObjectSchema, getSchema?: ObjectSchema) {
    for (const { putSubType, getSubType } of getDiscriminatedSubTypes(putSchema, getSchema)) {
      ...
      (objectType as unknown as ObjectType).properties[discriminatedObjectType.discriminator] = createObjectProperty(
